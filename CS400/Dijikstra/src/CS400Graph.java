...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
  ...
  protected Path dijkstrasShortestPath(NodeType start, NodeType end) throws NoSuchElementException {
    ...
    if (containsVertex(start) == false || containsVertex(end) == false) {
      throw new NoSuchElementException("No Vertex Is Found for One of Inputs");
    }

    Vertex firstNode = vertices.get(start);
    Vertex lastNode = vertices.get(end);
    PriorityQueue <Path> trackedPaths = new PriorityQueue<>();
    trackedPaths.add(new Path(firstNode));

    //Hashset to track is visited or not <Node, Boolean> (maybe set instead?)
    Set<Path> visited = new HashSet<>();

    while (!(trackedPaths.isEmpty())) {

      //Get first priority item
      Path currPath = trackedPaths.remove();

      //Priority queue makes sure the first way to get to that node from start is shortest
      if (currPath.end.equals(lastNode)) {
        return currPath;
      }

      //If visited skip, because we have already found a lower cost method to reach that point
      if (!visited.contains(currPath)) {
        //if not visited:
        //set visited to true
        visited.add(currPath);

        //Created extended path for each edge from currPath's end node add to priority queue new path
        for (int i = 0; i < currPath.end.edgesLeaving.size(); ++i) {
          //Insert to priority queue with extended path
          trackedPaths.add(new Path(currPath, currPath.end.edgesLeaving.get(i)));
        }
